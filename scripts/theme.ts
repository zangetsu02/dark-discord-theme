...
import { DiscordTheme } from './colors'
...
export default function getTheme(options: GetThemeOptions) {
  ...
  const secondaryBackground = v('secondaryBackground')
  ...
  const theme = {
    ...
    colors: {
      ...
      'terminal.ansiBlack': pick({ light: DiscordTheme.background[0], dark: DiscordTheme.foreground[1] }),
